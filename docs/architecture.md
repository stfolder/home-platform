...
The xrdp listener is an internal administrative service. It must be restricted to trusted LAN and WireGuard source networks and must never be forwarded directly from the public internet. The current implementation is not fully accepted until the effective privileged firewalld policy is captured. XFCE remains the validated recovery desktop, and the user-scoped i3 profile is now the active lightweight RDP optimization with XFCE preserved as rollback.
